CSES
        ...
        Sender:	jitu7033
        Submission time:	2023-12-05 14:48:24 +0200
        Language:	Java
        Status:	READY
        Result:	ACCEPTED
        Test results
        ...
        Compiler report
        Note: input/test.java uses unchecked or unsafe operations.
...
//         while (T-- > 0) {
        ...
        for(int i=1; i<=n; i++){
            for(int j=0; j<=sum; j++){
                if(j-arr[i-1]>=0){
                    dp[j] = Integer.min(dp[j],dp[(int) (j-arr[i-1])]+1);
                }
            }
        }
//             for(int i=0; i<=sum; i++){
...
Test 1
        ...
        input
        10 1000000
        2390 24239 49476 189008 194217...
